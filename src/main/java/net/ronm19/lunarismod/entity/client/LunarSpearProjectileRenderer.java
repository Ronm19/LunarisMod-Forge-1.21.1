...
public class LunarSpearProjectileRenderer extends EntityRenderer<LunarSpearProjectileEntity> {
    private NoctriumTomahawkProjectileModel model;

    public LunarSpearProjectileRenderer( EntityRendererProvider.Context pContext ) {
        super(pContext);
        this.model = new NoctriumTomahawkProjectileModel(pContext.bakeLayer(NoctriumTomahawkProjectileModel.LAYER_LOCATION));
    }

    @Override
    public void render( LunarSpearProjectileEntity pEntity, float entityYaw, float partialTicks, PoseStack poseStack, MultiBufferSource buffer, int packedLight ) {
        poseStack.pushPose();
    ...
    @Override
    public ResourceLocation getTextureLocation( LunarSpearProjectileEntity pEntity ) {
        return ResourceLocation.fromNamespaceAndPath(LunarisMod.MOD_ID, "textures/entity/lunar_spear/lunar_spear.png");
    }
}
